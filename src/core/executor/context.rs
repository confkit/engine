...
impl ExecutionContext {
    ...
    pub async fn new(
        ...
        let workspace_dir = format!("{}-{}-{}", space_name, project_name, task_id);
        let artifacts_dir = format!("{}-{}-{}", space_name, project_name, task_id);

        let host_workspace_dir = format!("{}/{}", HOST_WORKSPACE_DIR, workspace_dir);
        let host_artifacts_dir = format!("{}/{}", HOST_ARTIFACTS_DIR, artifacts_dir);
        let container_workspace_dir = format!("{}/{}", CONTAINER_WORKSPACE_DIR, workspace_dir);
        let container_artifacts_dir = format!("{}/{}", CONTAINER_ARTIFACTS_DIR, artifacts_dir);
...
impl ExecutionContext {
    ...
    fn build_environment(
        ...
        // 主机产物目录
        env.insert("HOST_ARTIFACTS_DIR".to_string(), host_artifacts_dir.to_string());
        // 容器工作空间目录
        env.insert("CONTAINER_WORKSPACE_DIR".to_string(), container_workspace_dir.to_string());
        // 容器产物目录
        env.insert("CONTAINER_ARTIFACTS_DIR".to_string(), container_artifacts_dir.to_string());

        // Git 相关变量
        if let Some(source) = &project_config.source {
            env.insert("GIT_REPO".to_string(), source.git_repo.clone());
            env.insert("GIT_BRANCH".to_string(), source.git_branch.clone());
        }
